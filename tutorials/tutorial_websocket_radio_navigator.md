...
- WebSocket enabled (default ON) on port 8765
- A modern web browser

## What You'll Learn
- How to connect to the WebSocket bidirectional interface
- How to parse telemetry for position and radio data
- How to send JSON commands to tune COM/NAV
- How to build a location‑aware radio manager in one HTML file

## Step 1: Create the HTML file

Create `web/radio_navigator.html` with the following content (already added to the repo). Note: the example supports canonical names with index fallbacks and can send commands by name or by index. Radios may expect values in MHz or Hz depending on the bridge build; the example handles both.
...
## Step 2: Run it
1. Start Aerofly FS4 with the Bridge DLL
2. Open `web/radio_navigator.html` in your browser
3. Take off and fly near one of the demo airports below — watch COM1 auto‑tune to Tower and NAV1 to the nearest VOR

## How it works

### WebSocket bidirectional protocol
- Connects to `ws://localhost:8765`
- Receives telemetry frames with `frame.variables["Canonical.Name"]`
- Sends JSON commands on the same socket:

```json
{ "variable": "Communication.COM1StandbyFrequency", "value": 119.10 }
```

Then swap to active:

```json
{ "variable": "Communication.COM1FrequencySwap", "value": 1 }
```

### Canonical variables used
- Position: `Aircraft.Latitude` (radians), `Aircraft.Longitude` (radians), `Aircraft.Altitude` (m)
- Radios:
  - COM: `Communication.COM{1,2}Frequency`, `Communication.COM{1,2}StandbyFrequency`, `Communication.COM{1,2}FrequencySwap`
  - NAV: `Navigation.NAV{1,2}Frequency`, `Navigation.NAV{1,2}StandbyFrequency`, `Navigation.NAV{1,2}FrequencySwap`

### Demo airports to test nearby

Stand near any of these demo airports for a quick test (included in the HTML):

- EBBR — Brussels National (BE)
- EBCI — Brussels South Charleroi (BE)
- LFQQ — Lille Lesquin (FR)
- EHAM — Amsterdam Schiphol (NL)
- EDDK — Cologne Bonn (DE)
- EHBK — Maastricht Aachen (NL)
- EBKT — Kortrijk–Wevelgem (BE)
- US set: KJFK, KLGA, KEWR, KBOS, KORD, KLAX, KSFO, KDEN, KMIA, KATL

Nearby VORs around those fields include: BUB, LIL, LNO, RKN (EU) and JFK, LGA, BOS, ORD, LAX, SFO, DEN, MIA (US).

### Location logic
- Convert lat/lon radians → degrees for display
- Compute great‑circle distance (Haversine) in NM
- Use a small embedded demo database (10 airports, 8 VORs) to determine nearest facilities

### Auto‑tune rules (simple demo)
- If within 50 NM of a known airport, set COM1 to Tower (rate‑limited)
- If a VOR is within 100 NM, set NAV1 to that VOR (rate‑limited)
- Manual buttons are always available

## Customization ideas
- Replace the demo airport/NAVAID lists with your own dataset or fetch from a service
- Add ATIS or Approach quick‑tune buttons
- Draw a simple moving map with the aircraft and navaid positions

## Troubleshooting

### NAV radios revert after swap?
- Some aircraft revert NAV active frequencies shortly after a programmatic swap (also seen when pressing the cockpit swap quickly). The example mitigates this by setting the standby first and swapping after a short delay. If it still reverts, try a slightly longer delay (80–120 ms) or re‑apply once after a debounce period.
- Connection fails: ensure WebSocket is enabled (`AEROFLY_BRIDGE_WS_ENABLE=1`) and port 8765 is open
- No data: start a flight and verify `data_valid` is 1 in frames
- Commands not applied: check variable names and value ranges; confirm your aircraft’s radios allow that frequency

---

You now have a location‑aware radio navigator running entirely in the browser, using the same bidirectional JSON protocol as desktop apps — ideal for quick tools and tablet EFBs.
